...
async function main() {
    try {
        console.log(`🔍 Fetching KaTeX v${LATEST_VERSION} manifest...`);
        ...
        console.log(`found ${uniqueFonts.length} unique fonts. Starting download...`);

        // 3. Download each font
        const downloads = uniqueFonts.map(async (fontName) => {
            try {
                await downloadFile(`${BASE_URL}fonts/${fontName}`, path.join(FONTS_DIR, fontName));
